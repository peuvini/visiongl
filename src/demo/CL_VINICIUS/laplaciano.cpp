/******************************************************************************
 * demo_laplaciano_cl.cpp
 * Usa o kernel OpenCL vglClLaplaciano para aguçamento de imagens.
...
int main(int argc, char* argv[])
{
    ...
    /*--------------- primeira chamada -------------------------------------*/
    TimerStart();
    vglClLaplaciano(img, out); // Chamada da função Laplaciano
    vglClFlush();
    printf("Primeira chamada ao Laplaciano: %s s\n", getTimeElapsedInSeconds());

    /*--------------- loop de benchmark ------------------------------------*/
    TimerStart();
    for (int i = 0; i < nSteps; ++i)
        vglClLaplaciano(img, out); // Chamada da função Laplaciano no loop
    vglClFlush();
    printf("Tempo em %d iterações: %s s\n", nSteps, getTimeElapsedInSeconds());

    /*--------------- salva resultado --------------------------------------*/
    vglCheckContext(out, VGL_RAM_CONTEXT);
    std::snprintf(outFile, sizeof(outFile), "%s/out_cl_laplaciano.tif", outPath);
